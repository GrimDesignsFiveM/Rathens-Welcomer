...
rathens.on("ready", () => {
  console.log(
    `${rathens.user.username} has started, with ${rathens.users.size} users, in ${rathens.channels.size} channels of ${rathens.guilds.size} guilds.`
  );
  timers.setInterval(() => {
...
rathens.on("guildMemberAdd", member => {
  ...
  member.guild.fetchInvites().then(guildInvites => {
    ...
    let iEmbed = new Discord.RichEmbed()
      ...
      .setThumbnail("https://images-ext-2.discordapp.net/external/_63laMY2YKORApnrpmUr3A1sywjCVNI1WNUGKSnMHYA/https/cdn.discordapp.com/icons/627932756197900332/f870b6cc103b3bb0c25ffc2c3f86474f.jpg?size=2048")
...
rathens.on("guildMemberAdd", member => {
  ...
  const channel = member.guild.channels.find(
    chnl => chnl.name === `${welcome_channel}`
  );
...
rathens.on('guildMemberAdd', member => {
  ...
  if(!member.guild.roles.find("name", "Members")) console.log("Members role not found");
  member.addRole(role);
});
